...
// VARIÁVEIS DE ULTRAPASSAGENS (PERSISTENTES)
let totalPasses = 0;           
...
sfxChuva.loop = true; sfxChuva.volume = 0.5; 
...
function saveProgress() {
    const gameData = {
        ...
        hasPlayedGoalMedia, dayPassesBest, totalPassesBest,
        totalPassesOdometer
    };
...
function resetDay() {
    ...
    videoDerrota.style.display = 'none'; videoDerrota.pause();
...
function drawF1Car(x, y, scale, color, isPlayer = false, nightMode = false, hasFog = false, isRainy = false) {
    ...
    if (nightMode || hasFog || isRainy) {
        ctx.fillStyle = "#FF0000"; ctx.fillRect(-w * 0.35, h * 0.2, w * 0.15, h * 0.25); 
        ...
        ctx.lineTo(w * 0.8, -lightLength); ctx.lineTo(w * 0.15, 0); ctx.fill();
    }
    if (!(nightMode || (hasFog && !isRainy))) {
...
function update() {
    ...
    if (gameState === "WIN_DAY" || gameState === "GAME_OVER") { 
        sfxChuva.pause(); draw(colors, isRaining); requestAnimationFrame(update); return; 
    }

    gameTick++; 
    if (speed > 0) {
        let delta = (speed / 10);
        playerDist += delta; odometerNow += delta; 
    }
    currentTime++; 
    ...
    // UI EXTERNA
    if(document.getElementById('ui-dist')) document.getElementById('ui-dist').innerText = (playerDist / 100).toFixed(1);
    if(document.getElementById('ui-day-best')) document.getElementById('ui-day-best').innerText = (dayBestRecord / 100).toFixed(1);
    if(document.getElementById('ui-total-now')) document.getElementById('ui-total-now').innerText = (odometerNow / 100).toFixed(1);
    if(document.getElementById('ui-total-best')) document.getElementById('ui-total-best').innerText = (totalBestRecord / 100).toFixed(1);
    ...
    if (isRaining || warningLightning) {
        ...
        if (Math.random() > 0.996) { lightningAlpha = 0.7; if (audioCtx.state === 'running') sfxTrovao.play().catch(e => {}); }
    } else { sfxChuva.pause(); }
    ...
    if (carsRemaining <= 0 && !hasPlayedGoalMedia) {
        hasPlayedGoalMedia = true; carsRemaining = 0; 
    ...
    if (currentTime >= DAY_DURATION) {
        if (carsRemaining <= 0) {
            if (gameState !== "WIN_DAY") { gameState = "WIN_DAY"; dayNumber++; saveProgress(); setTimeout(() => { resetDay(); }, 4000); }
        } else { if (gameState !== "GAME_OVER") { gameState = "GAME_OVER"; sfxDerrota.play(); videoDerrota.style.display = 'block'; videoDerrota.play().catch(e => {}); } }
    ...
    } else if (keys.ArrowDown) { speed = Math.max(speed - 0.2, 0); }
    ...
    enemies.forEach((enemy) => {
        ...
        if (p > 1.0 && !enemy.isOvertaken) { 
            enemy.isOvertaken = true; totalPasses++; totalPassesOdometer++; 
    ...
    if (gameTick % 240 === 0 && enemies.length < 100) {
        enemies.push({ 
            lane: (Math.random() - 0.5) * 1.8, z: 4000, v: 9.5, 
            color: ["#F0F", "#0FF", "#0F0", "#FF0"][Math.floor(Math.random() * 4)],
            isOvertaken: false 
        });
    }

    // --- AJUSTE AQUI: Inimigos agora somem logo após saírem da tela (Z < -500) ---
...
function draw(colors, isRaining) {
    ...
    for (let i = 200; i < 400; i += 4) {
        let p = (i - 200) / 140; let x = (200 - playerX * 0.05) + (roadCurve * p * p) - (playerX * p);
        let w = 20 + p * 800;
        ctx.fillStyle = Math.sin(i * 0.5 + playerDist * 0.2) > 0 ? (colors.nightMode ? "#050505" : "#333") : (colors.nightMode ? "#0a0a0a" : "#3d3d3d");
    ...
    ctx.fillStyle = (hasPlayedGoalMedia) ? "lime" : "yellow"; ctx.font = "bold 18px Courier";
    ctx.fillText((hasPlayedGoalMedia) ? "CARS: 000" : `CARS: ${carsRemaining}`, 15, 35);
