...
function updateUI() {
    ...
    // Atualização dos placares de ultrapassagem
...
function drawF1Car(x, y, scale, color, isPlayer = false, nightMode = false, hasFog = false, isRainy = false) {
    ...
    if(isPlayer) {
        ctx.rotate((roadCurve / 80) * Math.PI / 180);
    }
...
function update() {
    ...
    if (isRaining || warningLightning) {
        if (isRaining && sfxChuva.paused && audioCtx.state === 'running') {
            sfxChuva.play().catch(e => console.log("Erro audio")); 
        }
    ...
    if (currentTime >= DAY_DURATION) {
        if (gameState === "GOAL_REACHED" || carsRemaining <= 0) {
            if (gameState !== "WIN_DAY") { 
                ...
                videoVitoria.play().catch(e => console.log("Video vitoria erro"));
                dayNumber++;
                setTimeout(() => {
                    videoVitoria.style.display = 'none';
                    resetDay();
            ...
            if (gameState !== "GAME_OVER") { 
                ...
                videoDerrota.play().catch(e => console.log("Video derrota erro"));
            }
        }
    }
    ...
    if (--curveTimer <= 0) { 
        if (Math.random() > 0.6) {
            ...
            targetCurve = (Math.random() - 0.5) * 160;
            curveTimer = 80 + Math.random() * 150;
        }
    }
    roadCurve += (targetCurve - roadCurve) * curveSpeed;

    // --- LÓGICA DE INIMIGOS E PLACAR ---
    enemies.forEach((enemy, index) => {
        let currentV = (speed < 0) ? enemy.v * 0.4 : enemy.v;
        enemy.z -= (speed - currentV);
        
        // CORREÇÃO: Carros só sumirão se estiverem muito distantes para a frente (4500)
        if (enemy.z > 4500 || enemy.z < -15000) {
        ...
        // CORREÇÃO DO PLACAR: Contagem de ultrapassagem ocorre quando o carro passa pelo Z=0 (atrás do jogador)
        if (gameState === "PLAYING" || gameState === "GOAL_REACHED") {
            if (enemy.z <= 0 && !enemy.isOvertaken) { 
                ...
                passTotalOdo = Math.max(0, passTotalOdo - 1); 
...
function draw(colors, isRaining, currentStage) {
    ...
    // --- MONTANHAS (RECUPERADAS) ---
    let mtShift = (roadCurve * 0.6);
    for (let i = -3; i < 9; i++) {
        let bx = (i * 100) + mtShift;
        ctx.fillStyle = colors.mt;
        ctx.beginPath();
        ctx.moveTo(bx - 70, 200);
        ctx.lineTo(bx, 130);
        ctx.lineTo(bx + 70, 200);
        ctx.fill();
        if (colors.snowCaps) {
            ctx.fillStyle = "white";
            ctx.beginPath();
            ctx.moveTo(bx, 130);
            ctx.lineTo(bx - 25, 155);
            ctx.lineTo(bx + 25, 155);
            ctx.fill();
        }
    }
    ...
    for (let i = 200; i < 400; i += 4) {
        ...
        let asphaltColor;
        if (isSnowStage) {
            asphaltColor = Math.sin(i * 0.5 + playerDist * 0.2) > 0 ? "#FFF" : "#E0E0E0";
        } else {
            if (Math.sin(i * 0.5 + playerDist * 0.2) > 0) {
                asphaltColor = colors.nightMode ? "#050505" : "#333";
            } else {
                asphaltColor = colors.nightMode ? "#0a0a0a" : "#3d3d3d";
            }
        }
        ctx.fillStyle = asphaltColor;
        ctx.fillRect(x - w/2, i, w, 4);

        let zebraColor = Math.sin(i * 0.5 + playerDist * 0.2) > 0 ? (colors.nightMode ? "#600" : "red") : (colors.nightMode ? "#888" : "white");
    ...
    enemies.sort((a, b) => b.z - a.z).forEach(enemy => {
        if (enemy.lastP > -3 && enemy.lastP < 0.92) {
            drawF1Car(enemy.lastX, enemy.lastY, enemy.lastP * 0.85, enemy.color, false, colors.nightMode, hasFog, isRaining);
        }
    });
    ...
    if (isRaining) {
        ctx.strokeStyle = "rgba(200, 210, 255, 0.51)";
        ctx.lineWidth = 1.2;
        raindrops.forEach(r => {
            ctx.beginPath();
            ctx.moveTo(r.x, r.y);
            ctx.lineTo(r.x + 1.5, r.y + 12);
            ctx.stroke();
        });
    }
