...
function update() {
    ...
    if (isRaining) {
        for (let i = 0; i < 5; i++) raindrops.push({ x: Math.random() * 400, y: Math.random() * 400, s: Math.random() * 10 + 10 });
    }
...
function draw(colors) {
    ...
    if (colors.sky === "#4B0082" || colors.fog === 0.3) {
        ...
        raindrops.forEach(r => { ctx.beginPath(); ctx.moveTo(r.x, r.y); ctx.lineTo(r.x + 1, r.y + 15); ctx.stroke(); });
    }
    ...
    if (isPaused) {
        ...
        ctx.fillStyle = "white"; ctx.textAlign = "center"; ctx.font = "30px Courier"; ctx.fillText("PAUSADO", 200, 200);
        ctx.textAlign = "left";
    }
}
update();
