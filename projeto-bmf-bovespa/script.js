...
// Listas
const LISTA_AGRO_BMF = "BGIK26.SA,CCMH26.SA,SJWN26.SA,ICFU26.SA,WDOG26.SA,CTPK26.SA,TRIH26.SA";
...
async function buscarCotacoesAgro() {
    try {
        ...
        const data = await res.json();
        if (data.results) data.results.forEach(item => renderizarLinhaTabela(item, "BMF"));
    } catch (e) { console.error(e); }
}
    ...
    try {
        ...
        if (dataPrecos.results) {
            dataPrecos.results.forEach(item => {
                if (!LISTA_AGRO_BMF.includes(item.symbol)) renderizarLinhaTabela(item, "B3");
            });
        }
...
function renderizarLinhaTabela(item, origem) {
    ...
    const nomeExibicao = MAPA_NOMES_AGRO[prefixo] || item.shortName || tickerLimpo;
    const preco = item.regularMarketPrice || item.price || 0;
    const variacao = item.regularMarketChangePercent || item.changePercent || 0;
    const classeCor = variacao >= 0 ? "texto-alta" : "texto-queda";
    const classeSetor = origem === "BMF" ? "setor-bmf" : "setor-b3";

    tbody.innerHTML += `
        <tr class="${classeSetor}">
            <td class="col-ativo"><b>${nomeExibicao}</b><span class="nome-empresa-agro">${tickerLimpo} | ${origem}</span></td>
            <td class="col-preco">R$ ${preco.toFixed(2)}</td>
            <td class="col-pct ${classeCor}">${(variacao >= 0 ? '+' : '')}${variacao.toFixed(2)}%</td>
        </tr>`;
}

// --- RANKING (ALINHADO: ESQUERDA, CENTRO, DIREITA) ---
function processarRanking(dataRanking) {
    if (dataRanking && dataRanking.stocks) {
        ...
        const formatLi = (a, c) => `
            <li style="display: flex; justify-content: space-between; padding: 10px 5px; border-bottom: 1px solid rgba(0,0,0,0.05);">
                <span style="flex: 1; text-align: left; font-weight: bold;">${a.stock}</span>
                <span style="flex: 1; text-align: center; color: #666;">R$ ${a.close.toFixed(2)}</span>
...
// --- CARTEIRA E RENTABILIDADE (RESTAURADO) ---
function atualizarPainelCarteira(dadosApi) {
    ...
    minhaCarteira.forEach((item, index) => {
        ...
        tbody.innerHTML += `<tr class="${cor}"><td><b>${item.ticker}</b><br><small style="opacity:0.7">${nomeEmpresa}</small></td><td>R$ ${item.precoPago.toFixed(2)}</td><td>${precoAtual ? 'R$ ' + precoAtual.toFixed(2) : '---'}</td><td style="font-weight:bold">${pct}</td><td><button onclick="removerDaCarteira(${index})">🗑️</button></td></tr>`;
    });
}
...
function calcularRentabilidade() {
    ...
    const calcCDB = (v, t) => (v * (CDI / 100 / t)) * 0.775;
    const calcLCI = (v, t) => (v * ((CDI * 0.9) / 100 / t));
    container.innerHTML = `
        <div class="card-investimento"><h4>CDB (100% CDI)</h4><p>Mensal: <strong>R$ ${calcCDB(valor, 12).toFixed(2)}</strong></p></div>
        <div class="card-investimento"><h4>LCI/LCA (90% CDI)</h4><p>Mensal: <strong>R$ ${calcLCI(valor, 12).toFixed(2)}</strong></p></div>`;
}
...
function filtrarTabela(tipo) {
    ...
    linhas.forEach(l => l.style.display = (tipo === 'todos' || l.classList.contains('setor-' + tipo)) ? "" : "none");
}
function renderizarGrafico(dados) {
    ...
    chartMercado = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{ label: '% Var', data: dados.map(a => a.change || 0), backgroundColor: dados.map(a => (a.change || 0) >= 0 ? '#27ae60' : '#e74c3c') }]
        },
...
function calcularRentabilidade() {
    ...
    const SELIC = 11.25; const CDI = SELIC - 0.10; const diasUteisAno = 252; const mesesAno = 12;
    ...
    container.innerHTML = `<div class=\"card-investimento\"><h4>CDB / RDB (100% CDI)</h4><p>Diário: <strong class=\"texto-alta\">R$ ${calcCDB(valor, diasUteisAno).toFixed(2)}</strong></p><p>Mensal: <strong>R$ ${calcCDB(valor, mesesAno).toFixed(2)}</strong></p><small>Líquido (Pós-IR)</small></div><div class=\"card-investimento\"><h4>LCI / LCA (90% CDI)</h4><p>Diário: <strong class=\"texto-alta\">R$ ${calcLCI(valor, diasUteisAno).toFixed(2)}</strong></p><p>Mensal: <strong>R$ ${calcLCI(valor, mesesAno).toFixed(2)}</strong></p><small>Isento de Imposto</small></div><div class=\"card-investimento\"><h4>Poupança</h4><p>Diário: <strong class=\"texto-alta\">R$ ${calcPoup(valor, diasUteisAno).toFixed(2)}</strong></p><p>Mensal: <strong>R$ ${calcPoup(valor, mesesAno).toFixed(2)}</strong></p><small>Referencial</small></div>`;
}